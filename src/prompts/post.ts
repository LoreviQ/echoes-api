...
export const POST_GENERATION = {
    PROMPT: (character: Character, events: Event[] = []): string => {
        ...
        const eventsString = events.length === 0 ? "No recent events" : JSON.stringify(events, null, 2);
        return `Generate a short social media post (max 280 characters) from the perspective of the following character, considering the recent events.
...
Remember to write ONLY the post content itself, matching the character's voice and personality, and staying under the character limit.
...
Keep the post concise and suitable for a microblogging platform. **The post MUST NOT exceed 280 characters.** Avoid lengthy paragraphs.

Your output MUST be ONLY the text content of the social media post itself.
Do NOT include any introductory phrases, explanations, labels (like "Post:"), greetings, or markdown formatting.
